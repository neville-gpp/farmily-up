...
/**
 * Service for managing parent emotional state tracking
 * Stores feeling entries with timestamps in AsyncStorage
 */
class ParentFeelingService {
  static STORAGE_KEY = 'parent-feeling.json';
  ...
  static async recordFeeling(feelingType) {
    try {
      ...
      if (!this.VALID_FEELINGS.includes(feelingType)) {
        throw new Error(`Invalid feeling type: ${feelingType}. Must be one of: ${this.VALID_FEELINGS.join(', ')}`);
      }

      // Determine category and color automatically
      const category = this.determineFeelingCategory(feelingType);
      const color = category ? this.FEELING_CATEGORIES[category].color : '#CCCCCC';

      const now = new Date();
      const feelingEntry = {
        id: now.getTime().toString(), // Timestamp-based unique ID
        feeling: feelingType,
      ...
      // Get existing feelings or initialize empty structure
      const existingData = await this.getFeelings();
      const updatedData = {
        feelings: [...existingData.feelings, feelingEntry]
      };

      // Save to user-specific storage
      const success = await DataNamespacing.setUserData(this.STORAGE_KEY, updatedData);
  ...
  static async recordFeelingWithCategory(feelingType, category, color) {
    try {
      ...
      if (!this.FEELING_CATEGORIES[category]) {
        throw new Error(`Invalid category: ${category}. Must be one of: ${Object.keys(this.FEELING_CATEGORIES).join(', ')}`);
      }
  ...
  static async getFeelings() {
    try {
      const parsedData = await DataNamespacing.getUserData(this.STORAGE_KEY, { feelings: [] });
      ...
      if (!parsedData || typeof parsedData !== 'object') {
        console.warn('Invalid feeling data structure (not an object), resetting to empty');
        await this.clearAllFeelings();
        return { feelings: [] };
      }

      if (!Array.isArray(parsedData.feelings)) {
        console.warn('Invalid feeling data structure (feelings not an array), resetting to empty');
        await this.clearAllFeelings();
        return { feelings: [] };
      }

      // Validate individual feeling entries and filter out invalid ones
      const validFeelings = parsedData.feelings.filter(feeling => {
        ...
        if (feeling.category && !this.FEELING_CATEGORIES[feeling.category]) {
      ...
      if (validFeelings.length !== parsedData.feelings.length) {
        console.log(`Cleaned ${parsedData.feelings.length - validFeelings.length} invalid feeling entries`);
        await this.saveCleanedFeelings({ feelings: validFeelings });
      }
  ...
  /**
   * Save cleaned feelings data (internal method)
   * @private
   */
  static async saveCleanedFeelings(data) {
    try {
      await DataNamespacing.setUserData(this.STORAGE_KEY, data);
    } catch (error) {
      console.error('Error saving cleaned feelings data:', error);
    }
  }
  ...
  static async clearAllFeelings() {
    try {
      return await DataNamespacing.removeUserData(this.STORAGE_KEY);
    } catch (error) {
    ...
    try {
      ...
      data.feelings.forEach(feeling => {
        if (feeling.category && categorizedFeelings[feeling.category]) {
          ...
          const category = this.determineFeelingCategory(feeling.feeling);
          if (category) {
  ...
  /**
   * Determine the category of a feeling based on its type (for legacy data migration)
   * @private
   * @param {string} feelingType - The feeling type
   * @returns {string|null} The category or null if not found
   */
  static determineFeelingCategory(feelingType) {
  ...
  static async migrateFeelingData() {
    try {
      ...
      const migratedFeelings = data.feelings.map(feeling => {
        if (!feeling.category || !feeling.color) {
          ...
          if (category) {
            ...
            // Handle feelings that don't map to any category
            return {
              ...feeling,
      ...
      if (migrationNeeded) {
        const updatedData = { feelings: migratedFeelings };
        await DataNamespacing.setUserData(this.STORAGE_KEY, updatedData);
...
export default ParentFeelingService;